...
export class OfferService {
  ...
  async findAll(ctx: RequestContext, options?: ListQueryOptions<Offer>, sellerId?: ID): Promise<PaginatedList<Offer>> {
    const queryBuilder = this.listQueryBuilder.build(Offer, options, {
      ctx,
    ...
    if (sellerId) {
      queryBuilder.andWhere('offer."sellerId" = :sellerId', { sellerId });
    }

    return queryBuilder.getManyAndCount().then(([items, totalItems]) => ({ items, totalItems }));
  }

  async findOne(ctx: RequestContext, id: ID, sellerId?: ID): Promise<Offer | null> {
    if (sellerId) {
  ...
  async findActiveForBuyer(ctx: RequestContext, sellerId?: ID): Promise<Offer[]> {
    ...
    if (ctx.activeUserId) {
      ...
      return offers.filter((offer) => {
        ...
        return offer.customerGroupFilters.some((group) => buyerGroupIds.includes(group.id));
      });
    }

    // Unauthenticated: only show public offers (no group filters)
    return offers.filter((offer) => !offer.customerGroupFilters || offer.customerGroupFilters.length === 0);
  }
  ...
  async update(ctx: RequestContext, input: UpdateOfferInput): Promise<Offer> {
    ...
    if (input.removeLineItemIds?.length) {
      await this.connection.getRepository(ctx, OfferLineItem).delete(input.removeLineItemIds as number[]);
    }
    ...
    if (input.updateLineItems?.length) {
      ...
      await this.connection
        ...
        .save(input.updateLineItems.map((u) => existing.lineItems.find((li) => li.id === u.id)!).filter(Boolean));
    }
  ...
  async getQuantityOrdered(ctx: RequestContext, offerLineItemId: ID): Promise<number> {
    const result = await this.connection.getRepository(ctx, OfferLineItem).query(
  ...
  async findOfferLineItemWithOffer(ctx: RequestContext, id: ID): Promise<OfferLineItem | null> {
    const lineItem = await this.connection.getRepository(ctx, OfferLineItem).findOne({
      where: { id },
  ...
  async validateOfferForBuyer(ctx: RequestContext, offer: Offer): Promise<void> {
    ...
    if (offer.customerGroupFilters?.length > 0) {
      ...
      const buyerGroupIds = await this.getBuyerGroupIds(ctx);
      const isInGroup = offer.customerGroupFilters.some((g) => buyerGroupIds.includes(g.id));
      if (!isInGroup) {
   ...
   */
  private async validateCustomerGroupFilters(ctx: RequestContext, groupIds: ID[], sellerId: ID): Promise<void> {
    ...
    for (const group of groups) {
      ...
      if (groupSellerId && groupSellerId !== String(sellerId)) {
        throw new UserInputError(`CustomerGroup '${group.name}' belongs to a different seller`);
      }
    }
  }
