...
<svelte:boundary>
	{#await activeOffers({}) then offers}
		...
		{/if}
	{/await}
	{#snippet pending()}<p class="text-muted-foreground mt-4">Loading offers...</p>{/snippet}
	{#snippet failed(e)}<p class="text-destructive mt-4">Error loading offers: {e instanceof Error ? e.message : 'Unknown error'}</p>{/snippet}
</svelte:boundary>
