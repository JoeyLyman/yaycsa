<script lang="ts">
	...
	const isSellerPage = $derived(pathSegments.length >= 1 && pathSegments[0] !== 'me' && pathSegments[0] !== undefined);
	...
	// Brand suffix is bold on home, seller page root, and /me itself.
	// On /me sub-pages (/me/offers etc.), brand fades and nav link is active.
		...
		<div class="flex items-center gap-4">
			...
			{:else}
				<nav class="flex gap-4">
					...
					<a
						href="/register"
						class="text-sm transition-colors text-foreground font-medium hover:text-foreground"
					>Register</a>
				</nav>
			{/if}
		</div>
